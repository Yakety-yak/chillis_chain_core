...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="cis"; }
echo 'cis() { "$HOME/core-token/packages/core/bin/run" "$@" --token="cis"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/Yakety-yak/chillis_chain_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Yakety-yak/chillis_chain_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@cis"
rm -rf "$HOME/.config/@ChillisChain"
rm -rf "$HOME/.config/cis-core"
